...
// DJS see f_framerate int f_frames_per_second = 85;
...
int my_render_init()
{
	...
	/*
	 * Get resolution and frame rate from the render ping response. 
	 */
	 
	render_get_parameters(&f_width, &f_height, &f_framerate);
	dprintf("render screen parameters: %dx%d@%d\n", f_width, f_height, (int)f_framerate);
	initialize_pixel_conversion(x_dimension_mm, y_dimension_mm, f_width, f_height, f_screen_distance_MM);	// Initializes to_pixels function properties
...
int my_exp_init()
{
	...
	{
		dprintf("Creating sliding window length %d\n", (long)((double)f_spin_window_ms/1000.0f*f_framerate));
		f_pslider = slider_create((long)((double)f_spin_window_ms/1000.0f*f_framerate));
	}
	else
	{
	...
	else 
	{
		...
		if (f_savecommands==1)
		{
			...
			{
				f_savecommandsbuffersize = 50 * f_framerate * f_ms_jump_avg / 1000 * f_jumps_per_trial;
			}
			else
			{
...
int generate_trial()
{
	...
	if (f_jumps_per_trial == 0)
	{
		sbs.last = f_trial_max_length_s * f_framerate;
		sbs.jmin = 0;
		sbs.tau = 0;
		sbs.guard_before = 0;
		sbs.guard_after = 0;
	...
	else
	{
		 ...
		 */
	 	...
	 	sbs.last = 0;
		sbs.jmin = (int)(f_ms_jump_min * 0.001 * f_framerate);
		sbs.tau = ((float)f_ms_jump_avg - sbs.jmin/f_framerate*1000.0) * 0.001f * (float)f_framerate;
		sbs.guard_before = (int)((float)f_ms_jump_guard_before * .001f * (float)f_framerate);
		sbs.guard_after = (int)((float)f_ms_jump_guard_after * .001f * (float)f_framerate);
	...
	nblips = (int)((float)(sbs.last + sbs.jmin + sbs.tau)/f_framerate * f_blip_rate);
	nblinks = (int)((float)(sbs.last + sbs.jmin + sbs.tau)/f_framerate * f_blink_rate);
	nustim = (int)((float)(sbs.last + sbs.jmin + sbs.tau)/(float)f_framerate * f_ustim_rate);
	...
	if (f_blip_rate > 0)
	{
		...
		blip_duration_frames = (int)((float)f_blip_duration_ms / 1000.0f * f_framerate);
		blip_guard_frames = (int)((float)f_blip_guard_ms / 1000.0f * f_framerate);
		...
		while (blipcounter < nblips && failcounter < 1000)
		{
			...
			*/
			...
			if ((iover=segtree_overlaps(ptree, pseg, 0, NULL)) == 0)
			{
				...
				/*f_all_actions[n_all_actions].fval = f_blip_max_ang_velocity_deg_per_sec * M_PI/180.0f / f_framerate * (ifuniv(1)*2-1);*/
				f_all_actions[n_all_actions].fval = f_blip_max_ang_velocity_deg_per_sec * M_PI/180.0f / f_framerate * nrand_sign(&f_seed);
	...
	if (f_blink_rate > 0)
	{
		...
		blink_duration_frames = (int)((float)f_blink_duration_ms / 1000.0f * f_framerate);
		blink_guard_frames = (int)((float)f_blink_guard_ms / 1000.0f * f_framerate);
	...
	if (f_ustim_rate > 0)
	{
		...
		ustim_duration_frames = (int)((float)f_ustim_duration_ms / 1000.0f * f_framerate);
		ustim_guard_frames = (int)((float)f_ustim_guard_ms / 1000.0f * f_framerate);
