/* dotmap.d, RF mapping using 2d flow fields
 * 
 * Current version updated to be compatable with new render and rex 7.8
 * SWE 09.20.07
 * 
 *
...
static int isfpon = 0, isdotconf = 0, window_on = 0;
...
int	fixx = 0,
	...
	radius = 100,/* radius of dotfield */
	speed = 100,	/* dot speed in tenths of degrees */
	ptsiz = 2;		/* point size in pixels */
...
void get_dot_param(DotStruct *pdot)
{
	...
	pdot->xorigin = to_pixels((float)fixx/10);
	pdot->yorigin = to_pixels((float)fixy/10);
	pdot->xsize = siz;
	pdot->ysize = siz;
	pdot->depth = 10;
	pdot->r = 255;
...
VLIST state_vl[] = {
  ...
  "speed(deg/10sec) ",		&speed, NP, NP, 0, ME_DEC,
