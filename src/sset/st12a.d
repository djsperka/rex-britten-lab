/* $Id: st12a.d,v 1.3 2008/10/22 22:34:34 devel Exp $  */
...
int generate_trial()
{
	...
	if (f_blip_rate > 0)
	{
		...
		while (blipcounter < nblips && failcounter < 1000)
		{
			PstSegment pseg;
			int iover;
			int blipframe = (int)((sbs.last-1) * (float)rand()/(float)RAND_MAX)+1;
	...
	if (f_blink_rate > 0)
	{
		...
		while (blinkcounter < nblinks && failcounter < 1000)
		{
			PstSegment pseg;
			int iover;
			int blinkframe = (int)((sbs.last-1) * (float)rand()/(float)RAND_MAX)+1;
...
void queue_actions(int frame)
{
	...
	{
		dprintf("Q:%s", action_str(f_ordered_actions[f_candidate_action_index], s));
...
void dequeue_actions(int frame)
{
	...
	for (i=0; i<n_current_actions; i++)
	{
		...
		{
			/*dprintf("D:%s", action_str(f_current_actions[i], s));*/
