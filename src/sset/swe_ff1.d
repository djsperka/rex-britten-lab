/* $Id: swe_ff1.d,v 1.2 2008/09/12 22:32:09 devel Exp $ */
...
int	fixx = 0,
	...
	stimwid = 10,
	stimht = 1000,
    bf_con = 0,    /* 0: normal ; 1: add one black screen on each eye pursuit condition  */
	remain,
...
float fpsize = 0.2,
	...
	velx,
	vely;
double translation = 1000.0,		// in mm per sec
	pathlength;
...
int f_went_cycles=0;
int f_trial_framecounter;
int f_trial_maxframes = 10000;
...
//	render_dot(&f_fpStruct3);		//will need to get handle return
	...
	dprintf("INIT\n");
	dprintf("f_fphandle %d %d\n", (int)f_fpStruct.xsize*100, (int)f_fpStruct.ysize*100);
	dprintf("f_fphandle2 %d %d\n", (int)f_fpStruct2.xsize*100, (int)f_fpStruct2.ysize*100);
	dprintf("f_fphandle3 %d %d\n", (int)f_fpStruct3.xsize*100, (int)f_fpStruct3.ysize*100);


	// configure dotfield
	f_dotsStruct.xwidth = 10000.0;
	f_dotsStruct.ywidth = 10000.0;
	f_dotsStruct.zwidth = 10000.0;
	f_dotsStruct.xorigin = 0.0;
	f_dotsStruct.yorigin = 0.0;
	f_dotsStruct.zorigin = -5500;
...
int my_path_setup()
{
	...
	dprintf("pathdely = %d\tpathdelayframes%d\n",(int)pathdelay, (int)pathdelayframes);

	f_path.nframes = movieframes;	// 2 seconds at 85Hz
	f_path.ndelay = pathdelayframes;		// no delay before starting motion
...
begin	first:
	...
	frame:
		do render_frame(0)
		to showinit on 1 % render_check_for_went
	showinit:
		do my_show()
		to winon
	winon:
