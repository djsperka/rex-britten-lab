/* $Id: st12.d,v 1.4 2008/09/12 20:40:14 devel Exp $  */
...
"max_velocity(degrees/sec)", &f_blip_max_ang_velocity_deg_per_sec, 0, NP, 0, ME_FLOAT,
...
{
	sprintf(str, "%d-%d %d %f\n", paction->start_frame, paction->end_frame, paction->type, paction->fval); 
	return str;
}
...
int generate_trial()
{
	 ...
	 * and then we can calculate how many total actions are needed and allocate the space for them. 
	 * The total time for the trial should include a "tail" added after the last jump. That will be added when we add 
	 * the blips and blinks, but for now include the time in the rate calculations.
	 */
	 ...
	 * TODO will have to add an action for this.
	 */
	...
	if (f_blip_rate > 0)
	{
		...
		int nblips;					/* Number of blips to generate and add to trial */
		int blipcounter = 0;		/* Counter of blips added */
		int failcounter = 0;		/* Counter of failures to add blip...overlaps, get it? */
		
		nblips = (int)(f_blip_rate * sbs.last / f_frames_per_second);
		...
		while (blipcounter < nblips && failcounter < 1000)
		{
			PstSegment pseg;
			int iover;
			int blipframe = (int)(sbs.last * (float)rand()/(float)RAND_MAX);
	...
	if (f_blink_rate > 0)
	{
		...
		int nblinks;				/* Number of blinks to generate and add to trial */
		int blinkcounter = 0;		/* Counter of blinks added */
		int failcounter = 0;		/* Counter of failures to add blink...overlaps, get it? */
		
		nblinks = (int)(f_blink_rate * sbs.last / f_frames_per_second);
		...
		while (blinkcounter < nblinks && failcounter < 1000)
		{
			PstSegment pseg;
			int iover;
			int blinkframe = (int)(sbs.last * (float)rand()/(float)RAND_MAX);
...
int my_trial_init()
{
	...
	dprintf("f_num_targ_offset_degrees=%d\n", f_num_targ_offset_degrees);
