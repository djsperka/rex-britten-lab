...
int	fixx = 0,
    ...
    int f_windowXSize = 30;     /* rex eye window X size in tenths of a degree */
    int f_windowYSize = 30;     /* rex eye window Y size in tenths of a degree */
    int f_width;           /* render screen width in pixels */
    int f_height;          /* render screen height in pixels */
    double f_framerate;    /* render frame rate in hz */
...
int initial(void)
{
	...
	/* 
	 * Set up eye windows. Only need to do this once. 
	 * wd_src_pos sets WD_DIRPOS for both x and y -- that means 
	 * that calls to wd_pos() will determine where the eye window is set.
	 * wd_src_check tells REX what to compare (i.e. what the 'current' eye 
	 * position is). We set it to a SIGNAL; EYEH_SIG and EYEV_SIG mean we
	 * are using the analog values 'eyeh' and 'eyev'. 
	 */
 ...
 * If 'on' is ASK_ON(=1), update the dot position and ask render to turn it on. Also, set
 * eye window position (wd_pos) and size (wd_siz) and turn on the eye window (wd_cntrl(WD_ON))
 * 
 * if 'on' is ASK_OFF (=0) ask render to turn off the dot. Also turn off the eye window (wd_cntrl(WD_OFF))
...
int targ_ask(int on, int which)
{
	...
	if (which == ASK_FP)
	{
		if (on == ASK_ON)
		{
			/* Position render dot at fixation location, set color and turn on */
			...
			/* Set position and turn on REX eye window */
			
			wd_siz(WIND0, (long)f_windowXSize, (long)f_windowYSize);
	...
	else if (which == ASK_TARG)
	{
		...
		if (on == ASK_ON)
		{
			...
			/* Decide where target location should be. */
			...
			/* update REX eye window position and turn ON */
			...
			/* update render dot to target location */
