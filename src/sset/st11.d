/* $Log: st11.d,v $
/* Revision 1.6  2008/07/30 21:21:27  devel
/* Shorten menu text for blip max vel.
/*
/* Revision 1.5  2008-02-29 00:51:11  devel
/* Add bcodes for parameters controlling paradigm in my_render_init(). These are issued after a RESET.
/*
/* Revision 1.4  2008-02-28 22:31:59  devel
/* Fix blips. Now sinusoidal, specified in angular velocity. ID=401
/* */
...
"Target_offset_1(deg)", &f_targ_offset_degrees[0], 0, NP, 0, ME_DEC,
"Target_offset_2(deg)", &f_targ_offset_degrees[1], 0, NP, 0, ME_DEC,
...
int my_update()
{
	...
	switch (blip_update(f_went_counter, &blip_delta))
	{
		case BLIP_BEGIN:
			ecode(BLIPBEGINCD);
			bcode_float(CH_BLIP_OMEGA_MAX, f_blip.omega_max[f_blip.index]);
			bcode_uint(CH_BLIP_DURATION_FRAMES, f_blip.endframe[f_blip.index] - f_blip.beginframe[f_blip.index]);
			break;
		case BLIP_IN_PROGRESS:
//			dprintf("%d ", (int)(blip_delta*180/M_PI*100));
...
/* blip_prepare()
 * 
 * This function is called at the beginning of each "jump" period. It must be called before blip_update() is called for 
 * that period. All parameters which control the blip, including whether any blips will occur (and how many), are determined
 * here and stored in the structure f_blip. 
 * 
 * These global variables are used:
 * 
 * f_blip_duration_ms - how long, in ms, is the blip?
 * f_blip_guard_ms - guard time, in ms. 
 * f_frames_per_second - used to convert the above to frames
 * f_blip_probability_per_jump - probability, on a scale 0-1000, where 1000 corresponds to 100%
 * f_frames_before_jump, f_frames_after_jump - used to determine the allowed times where blip can start
 * 
 */
...
void blip_prepare()
{
	...
	else
	{
		for (i=0; i<f_blip.num; i++)	
		{
			...
			int rval = ivunif(0, range_max);
			int ampsign = ivunif(0, 1)*2-1;

			/* assign amplitude and sign of blip angular velocity */
			f_blip.omega_max[i] = f_blip_max_ang_velocity_deg_per_sec * M_PI/180.0f / f_frames_per_second * ampsign;
...
int blip_update(int frame, float *poffset)
{
	...
	if (f_blip.num > 0 && f_blip.index >= 0)
	{
		...
		else if (frame > f_blip.beginframe[f_blip.index] && frame <= f_blip.endframe[f_blip.index])
		{
			...
			// compute offset - this is an angular displacement, in radians.
			// Using sinusoidal profile for angular velocity. 
			// Take value at midpoint of interval (that's where the "- 0.5" term comes from) and multiply it by 1 frame -- 
			// in effect we take the velocity as constant over the duration of this period. 
			//
			// Note also that the half-period of the sinusoid runs from the beginframe to the endframe. The numerator below
			// is the number of frames since the beginframe (less 0.5). The denominator is the number of frames in the blip, 
			// which is endframe-beginframe.  
			if (frame > f_blip.beginframe[f_blip.index])
			{
				float a = ((float)(frame -f_blip.beginframe[f_blip.index]) - 0.5f)/(float)(f_blip.endframe[f_blip.index]-f_blip.beginframe[f_blip.index]);
				offset = f_blip.omega_max[f_blip.index] * sin(a*M_PI);
//				dprintf("%d/%d %d\n", frame, f_blip.endframe[f_blip.index], f_blip.beginframe[f_blip.index]);
//				dprintf("(%d)", (int)(a*100));
			}
		}
	}
