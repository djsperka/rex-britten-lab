/* $Id: st15a.d,v 1.2 2008/10/28 23:28:43 devel Exp $  */
...
/* Spin handling stuff */
int f_spinning = 0;					/* Set to 1 when spinning is detected */
int f_spin_enable = 1;				/* if nonzero, spintest is applied */
int f_spin_time_ms = 2000;
int f_spin_queue_size = 0;
int f_spin_min_deviation = 10;
PSpinQ f_pspinq = NULL;
...
/* Spintest parameters menu */

VLIST spin_vl[] = {
"spin_test_enable", &f_spin_enable, NP, NP, 0, ME_DEC,
"spin_time(ms)", &f_spin_time_ms, NP, NP, 0, ME_DEC,
"min_deviation(adc)", &f_spin_min_deviation, NP, NP, 0, ME_DEC,
NS,
};
...
{"spin", &spin_vl, NP, NP, 0, NP, hm_spin}, 
...
int my_trial_init()
{
	...
	/* 
	 * Allocate and initialize spinq. Assuming that my_trial_done does 
	 * cleanup each trial (and that it is set as an abort action as well).  
	 * We check spin_enable here, and only allocate the spinq if it is set. 
	 * In my_update, the only test that is performed is to check whether
	 * f_pspinq is null - if spintest is enabled during a trial it will not
	 * be honored (until the next trial begins). 
	 */
	if (f_spin_enable)
	{
		f_pspinq = spinq_create(2048);
		f_spin_queue_size = (int)((float)f_spin_time_ms*f_frames_per_second/1000.0f);
		spinq_init(f_pspinq, f_spin_queue_size, f_spin_min_deviation);
	}
...
int my_trial_done()
{
	stpause(1);		/* this closes analog window */
...
int my_update()
{
	...
	if (f_pspinq)
	{
		int itemp = spinq_push(f_pspinq, ijoyh);
		if (itemp < 0)
		{
			dprintf("spinq_push returned %d, ijoyh=%d\n", itemp, ijoyh);
		}
		else
		{
			if (f_verbose & DEBUG_SPIN_BIT && itemp != f_spinning)
			{
				if (itemp==0) dprintf("Not spinning\n");
				else if (itemp==1) dprintf("Spinning\n");
			}
			f_spinning = itemp;
		}
	}
...
int stpause(int isPaused)
{
	if (isPaused)
	{
		...
		/* Clear spin queue if it exists. */
		if (f_pspinq)
		{
			spinq_clear(f_pspinq);
		}
	}
	else
	{
...
int reward_init()
{
	...
	if (f_verbose & DEBUG_REWARD_BIT) dprintf("reward_init going=%d spinning=%d\n", f_going, f_spinning);
}
 ...
 * return -2 on error (jump counters off)
 * 
 */ 
