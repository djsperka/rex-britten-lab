/* $Id: stttaf.d,v 1.1 2010/10/13 23:48:51 devel Exp $ */
...
int generate_trial()
{
	...
	if (f_meander_target_dwell_frames_min > 0)
	{
		...
		if (!f_pmbqf_meander_distribution)
		{
			rxerr("Cannot allocate memory for target meander distruribution!");
			return 1;
		}
		else
		{
...
int my_update()
{
	...
	uint64_t cycle1, cycle2, cps, ncycles;
	float seconds;
	...
	if (f_verbose & DEBUG_CLOCK_BIT)
	{
    	...
    	seconds=(float)(ncycles)/(float)cps;
    	dprintf("seconds %d \n",(int)(seconds*1000000));		
	}
...
int fixation()
{
	...
	dprintf("INCREMENT(AllTrials) returns %d\n", i);
	return 0;
}
